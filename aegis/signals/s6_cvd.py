...
if __name__ == "__main__":
    QUICK_TEST_WARMUP = WARMUP_SECONDS   # 2 min quick test (not full 50M — for dev only)

    print("Starting CVD WebSocket stream (v2.0 optimized)...")
    print(f"Quick-test warmup: {QUICK_TEST_WARMUP}s\n")
    print("Optimizations active:")
    print("  ✓ Wall-clock aligned 5M candle buckets")
    print("  ✓ Session CVD reset at 00:00 / 08:00 / 13:00 UTC")
    print("  ✓ Slope-based divergence (linear regression)")
    print("  ✓ Magnitude filters (price + CVD + strength)")
    print(f"  ✓ Multi-TF confirmation ({DIVERGENCE_SHORT*5}M + {DIVERGENCE_LONG*5}M)\n")

    calc = start_cvd_stream()

    for i in range(QUICK_TEST_WARMUP):
        time.sleep(1)
        filled = int((i + 1) / QUICK_TEST_WARMUP * 40)
        bar    = "█" * filled + "░" * (40 - filled)
        print(f"\r  [{bar}] {i+1}/{QUICK_TEST_WARMUP}s", end="", flush=True)

    print("\n")
    result = get_signal()

    print("=" * 65)
    print("  SIGNAL 6 — CUMULATIVE VOLUME DELTA (CVD)  v2.0")
    print("=" * 65)

    skip_keys = {"short_window", "long_window"}
    for k, v in result.items():
        if k not in skip_keys:
            print(f"  {k:<15} : {v}")

    print("\n  ── 25M Window ──")
    sw = result.get("short_window", {})
    print(f"  {'divergence':<15} : {sw.get('divergence', 'N/A')}")
    print(f"  {'score':<15} : {sw.get('score', 'N/A')}")
    print(f"  {'strength':<15} : {sw.get('strength', 'N/A')}")

    print("\n  ── 50M Window ──")
    lw = result.get("long_window", {})
    print(f"  {'divergence':<15} : {lw.get('divergence', 'N/A')}")
    print(f"  {'score':<15} : {lw.get('score', 'N/A')}")
    print(f"  {'strength':<15} : {lw.get('strength', 'N/A')}")
